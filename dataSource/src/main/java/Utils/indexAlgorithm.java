...
public class indexAlgorithm {
    ...
    public static indexNode indexDatasetKD(HashMap<String, ArrayList<Long>> datasetIdMapSignature, HashMap<String, indexNode> datalakeRootNodes, indexNode parentNode, int dimension, int leafSize, int upperGlobalId, String siloID, boolean isBatchUpdate) {
...
//        radius = Math.max(distance2(pivot,maxBox), distance2(pivot,minBox));
...
//                System.out.println("id = "+root.getNodeid()+", radius = " + root.radius);
            ...
            // 生成 posting list
            TreeMap<Long, HashSet<String>> pl = genInvertedIndex(dataset);
            a.setPostinglist(pl);
...
//                        System.out.println("id = "+root.getNodeid()+", radius = " + root.radius);
    ...
    public static TreeMap<Long, HashSet<String>> genInvertedIndex(HashMap<String, ArrayList<Long>> dataset) {
        TreeMap<Long, HashSet<String>> postinglist = new TreeMap<>();
    ...
    public void storeDatalakeIndex(indexNode root, String nodeid, String storePath) {
        ...
        if (root.getDatasetID() == -1) {  // leaf node
            for (String i: root.getNodeIDList()){
                contentString = contentString+ String.valueOf(i)+",";
            }
        } else { // internal node
